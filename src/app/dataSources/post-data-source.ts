...
export class AllPostsDataSource extends DataSource < Post | undefined > {
  ...
  constructor(private postService: PostService, numberOfPosts: number, searchString: string, username: string) {
    super()
  ...
  public async search(searchString: string) {
    this.searchString = searchString;
    let numberOfPosts = await this.postService.getPostsLengthFromUsersIFollow(this.searchString, this.username);
...
export class HashtagDataSource extends DataSource < Post | undefined > {
  private length;
  private pageSize = 5;
  private fetchedPages = new Set < number > ();
  private cachedData;
  ...
  constructor(private postService: PostService, numberOfPosts: number, hashtag: string, searchString: string) {
    super()
    this.searchString = searchString;
  ...
  public async search(searchString: string) {
    this.searchString = searchString;
    let numberOfPosts = await this.postService.getPostsOfHashtagLength(this.searchString, this.hashtag);
    this.length = numberOfPosts;
    this.fetchedPages = new Set < number > ();
    this.cachedData = Array.from < Post > ({
      length: this.length
  ...
  async fetchPage(page: number) {
    ...
    this.fetchedPages.add(page);
    let d = await this.postService.getPostsOfHashtag(this.searchString, this.hashtag, this.pageSize, page);
    this.cachedData.splice(page * this.pageSize, this.pageSize,
      ...Array.from < Post > ({
        length: d.length
      }).map((_, i) => {
